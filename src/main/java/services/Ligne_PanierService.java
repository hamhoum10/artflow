...
public class Ligne_PanierService {
    ...
    public void deleteAllFromLigne_panier(int id_panier) {
        try {
            ...
            String sql ="DELETE lp FROM ligne_panier lp JOIN article a ON a.id_article = lp.id_article JOIN panier p ON lp.id_panier = p.id_panier WHERE p.id_panier =  "+ id_panier;
            PreparedStatement p  = cnx.prepareStatement(sql);
            p.setInt(1, id_panier);
            p.executeUpdate();
