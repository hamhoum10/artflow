...
public class
CommandeService { Connection cnx = MyConnection.getInstance().getCnx();
...
public  List<Commande> selectName(){
...
//                panier.setId_panier(rs.getInt("id_panier"));
                ...
                commande.setTotalAmount( rs.getFloat("total_amount"));
                commande.setCreatedAt(String.valueOf(rs.getDate("created_at")));
                commande.setCodepostal(rs.getInt("codepostal"));
                commande.setAdresse(rs.getString("adresse"));
                commandeList.add(commande);
            }
    ...
    public int getCommendeIdByName(String s) throws SQLException {
    String query = "select `id` from commande where nom = '"+s+"'";
    int r = 0;
        try (PreparedStatement ps = cnx.prepareStatement(query)) {
    ...
    public String getNumById(String s) throws  SQLException{
        String query = "select `numero` from commande where nom ='"+s+"'";
        String n = "";
        try (PreparedStatement ps = cnx.prepareStatement(query)) {

            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                n = rs.getString("numero");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
